...
class ScriptInputs(BaseModel):
    ...
    isp: float = Field(..., description="Specific impulse in seconds")
    ...
    oxidizer_pressure_drop: float = Field(
        ..., description="Oxidizer pressure drop in Pascals"
    )
    oxidizer_velocity: float = Field(
        ..., description="Oxidizer injection velocity in m/s"
    )
    oxidizer_name: str = Field(..., description="Oxidizer chemical name")
    ...
    chamber_temperature: float = Field(..., description="Chamber temperature in Kelvin")
